...
# The official implementation of the Swin transformer from `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
# https://arxiv.org/pdf/2103.14030 code can be found at https://github.com/microsoft/Swin-Transformer/

# Some adaptations of the original code for use in grasp detection are included from 'When Transformer Meets Robotic Grasping: Exploits Context for Efficient Grasp Detection' 
# https://arxiv.org/abs/2202.11911 Code can be found at https://github.com/WangShaoSUN/grasp-transformer/
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=1, num_classes=1,
        ...
        if self.final_upsample == "expand_first":
            ...
            self.qual_output  = nn.Conv2d(in_channels=embed_dim, out_channels=self.num_classes, kernel_size=1, bias=False)
    ...
    def up_x4(self, x):
        ...
        if self.final_upsample == "expand_first":
            ...
            qual_output = self.qual_output(x)
...
if __name__ == '__main__':
    ...
    model = SwinTransformerSys(in_chans=4,embed_dim=24,num_heads=[3, 6, 12, 24]).cuda()
    ...
    len=20
    for i in range(len):
      imge = torch.rand(1, 4, 224, 224).cuda()
