...
def plot_output(rgb_img, grasp_q_img, grasp_angle_img, grasp_width_img, no_grasps=1):
    """
    ...
    :param grasp_width_img: Width output of network
    :return:
    """
    ...
    plot = ax.imshow(grasp_width_img, cmap='hsv', vmin=0, vmax=1)
    ax.set_title('Width')
    ax.axis('off')
    plt.colorbar(plot)
...
def alt_plot_output(rgb_img, grasp_q_img, grasp_angle_img, grasp_width_img, no_grasps=1):
    """
    Plot the output of a network
    :param fig: Figure to plot the output
    :param rgb_img: RGB Image
    ...
    ax = fig.add_subplot(2, 2, 3)
    plot = ax.imshow(grasp_width_img, cmap='brg', vmin=0, vmax=1)
    ax.set_title('Width')
    ax.axis('off')
    plt.colorbar(plot)
...
def calculate_iou_match(grasp_q, grasp_angle, ground_truth_bbs, no_grasps=1, grasp_width=None, threshold=0.25):
    ...
    A success is counted if grasp rectangle has a 25% IoU with a ground truth, and is within 30 degrees.
    :param grasp_q: Q outputs of network (Nx224x224x3)
